...
# The majority of this code is from https://github.com/Rostlab/EAT/blob/main/train_prottucker.py
...
class DataSplitter():
    def __init__(self, embedding_p, verbose=True):
        ...
        self.data_dir = embedding_p.parent.parent
        ...
        self.cath_label_path = self.data_dir / 'annotations' / 'Y_Train_Test_Val_concat_SF.txt'
    ...
    def get_precomputed_embeddings(self, embedding_p):
        # Load pre-computed embeddings in .npz file format
        npz_file = np.load(embedding_p)
        embeddings_array = npz_file['arr_0']  # Assuming the array is saved as 'arr_0'

        # Create a dataset dictionary using the sorted keys
    ...
    def get_embeddings(self, fasta_path):
        ...
        cath_ids = [int(cath_id) for cath_id in cath_ids]
        ...
        for cath_id in cath_ids:
            try:
                ...
                #DEBUG
                # print('len cath_ids: {}'.format(len(cath_ids)))
                # print('1st cath_ids: {}'.format(cath_ids[:10]))
                # print('len id2embedding: {}'.format(len(self.id2embedding)))
                # print('embeddings 0: {}'.format(self.id2embedding[0]))
                # print('embeddings 0: {}'.format(self.id2embedding[len(cath_ids)-1]))
                # print('id type: {}'.format(type(cath_id)))
                # print('id2embedding key type: {}'.format(type(list(self.id2embedding.keys())[0])))
                # exit()
    ...
    def get_predef_splits(self, p_train=None, p_test=None):
        ...
        if p_train is None or p_test is None:
            p_train = self.data_dir / 'csv' / "Train.fasta"
            p_val = self.data_dir / 'csv' / "Val.fasta"
            p_valLookup20 = self.data_dir / 'csv' / "Test.fasta"
...
class Saver():
    ...
    def save_model_h5(self, model):
        h5_path = './saved_models/ann_ProstT5_full_CL.h5' 
...
def main():
    ...
    # embedding_p = './data/Dataset/embeddings/Train_Val_ProstT5_full_concat.npz'
    # npz_file = np.load(embedding_p)
    # embeddings_array = npz_file['arr_0']

    # # Print the first few keys and embeddings to verify
    # print("Number of embeddings:", len(embeddings_array))
    # print("Example keys and embeddings:")
    # for idx, emb in enumerate(embeddings_array[:5]):
    #     print(f"Key: {idx}, Embedding: {emb.shape}")

    # # Ensure the key count matches the number of embeddings
    # assert len(embeddings_array) == max(embeddings_array.shape)

    # # Path to labels file
    # label_path = './data/Dataset/annotations/Y_Train_Val_concat_SF.txt'

    # # Read the labels file and collect IDs
    # ids_from_labels = []
    ...
    #         data = line.split()
    #         identifier = data[0]
    #         ids_from_labels.append(int(identifier))

    # # Print the first few IDs to verify
    # print("Number of IDs in labels:", len(ids_from_labels))
    # print("Example IDs from labels:", ids_from_labels[:5])

    # # Ensure that the IDs from the labels are within the range of embedding keys
    # assert max(ids_from_labels) < len(embeddings_array)
    ...
    embedding_p = data_dir / "Dataset" / "embeddings" / "Train_Test_Val_ProstT5_full_concat.npz" # path to your pre-computed embeddings. IDs have to align with label IDs
    print("Loading dataset from: {}".format(embedding_p))

    # give your experiment a meaningful name here
    experiment_name = "1st contrastive learning training: embeddings from ProstT5_full"
    ...
    for epoch in range(num_epochs):  # for each epoch
        ...
        start = time.time()
        for train_idx, (X, Y, _) in tqdm(enumerate(train), total=len(train)):  # for each batch in the training set
            X = X.to(device)
            Y = Y.to(device)
